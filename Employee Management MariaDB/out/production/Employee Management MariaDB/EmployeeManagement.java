...
public class EmployeeManagement {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println((int)employees.stream().map(Employee::getSalary).map(i->Integer.valueOf(i)).min(Integer::compare).orElse(0));
            ...
            System.out.println(employees.stream().filter(i->i.getJoiningDate().isBefore(febEnd)&&i.getJoiningDate().isAfter(febStrart)).map(Employee::getSalary).map(i->Integer.valueOf(i)).mapToInt(i->i).sum());


            LocalDate febMid = LocalDate.of(2021,5,14);
            System.out.println("\n\nAverage salary for employee joined on 14-Mar-2021");
            System.out.println((int)employees.stream().filter(i->i.getJoiningDate().isEqual(febMid)).map(Employee::getSalary).map(i->Integer.valueOf(i)).mapToInt(i->i).average().orElse(0));
            ...
            System.out.println(employees.stream().collect(Collectors.groupingBy(Employee::getSalary,Collectors.counting())));
            ...
            System.out.println(employees.stream().collect(Collectors.groupingBy(Employee::getDepartment,Collectors.mapping(Employee::getEmployee,Collectors.toList()))));


            System.out.println("\n\na data map with department and their salary");
//            int =0,qaSalary=0,itSalary=0,managerSalary=0;
            System.out.println(employees.stream().collect(Collectors.groupingBy(Employee::getDepartment,Collectors.mapping(Employee::getSalary,Collectors.toList()))));
